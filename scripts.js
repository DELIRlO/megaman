...
if(window.audioSystem){window.audioSystem.play('menuSelect');}}
function closeMenu(){
overlay.classList.remove('show');
document.body.style.overflow='';
if(window.audioSystem){window.audioSystem.play('click');}}
shortcut.addEventListener('click',openMenu);
shortcut.addEventListener('touchstart',(e)=>{e.preventDefault();openMenu();});
closeButton.addEventListener('click',closeMenu);
closeButton.addEventListener('touchstart',(e)=>{e.preventDefault();closeMenu();});
...
item.addEventListener('touchstart',()=>{if(window.audioSystem){window.audioSystem.play('menuHover');}});
...
item.addEventListener('touchstart',()=>{if(window.audioSystem){window.audioSystem.play('menuHover');}});
item.addEventListener('click',()=>{if(window.audioSystem){window.audioSystem.play('click');}});});}

function initializeAudioControls(){
const muteBtn=document.getElementById('mute-btn');
if(muteBtn){
muteBtn.addEventListener('click',()=>{if(window.audioSystem){window.audioSystem.toggleMute();}});}}
...
if(!isMuted){console.log('🎵 Música de fundo iniciada');}},500);},3000);}

function initializeMenu(){
const menuItems=document.querySelectorAll('.menu-item');
menuItems.forEach(item=>{
item.addEventListener('mouseenter',()=>{if(window.audioSystem){window.audioSystem.play('menuHover');}});
item.addEventListener('click',()=>{
if(window.audioSystem){window.audioSystem.play('menuSelect');}
const page=item.dataset.page;
...
link.addEventListener('mouseenter',()=>{if(window.audioSystem){window.audioSystem.play('menuHover');}});
link.addEventListener('click',()=>{if(window.audioSystem){window.audioSystem.play('click');}});});}
...
if(!targetPage){targetPage=createPage(pageName);}
targetPage.classList.add('active');
if(window.audioSystem){window.audioSystem.play('teleport');}},1000);}
...
page.className='page';
switch(pageName){
case'sobre':page.innerHTML=createSobrePage();break;
case'curriculo':page.innerHTML=createCurriculoPage();break;
case'projetos':page.innerHTML=createProjetosPage();break;
case'skills':page.innerHTML=createSkillsPage();break;
case'blog':page.innerHTML=createBlogPage();break;
case'contato':page.innerHTML=createContatoPage();break;
default:page.innerHTML='<h1>PÁGINA EM CONSTRUÇÃO</h1><p>Esta página está sendo desenvolvida...</p>';}
...
setTimeout(()=>{megamanTransition.classList.remove('active');},2000);}
...
this.value='';
return;}
else{output.innerHTML+='<br>'+commands[command];}}
else{output.innerHTML+='<br>Comando não reconhecido.Digite"help"para ver os comandos disponíveis.';}
...
setTimeout(()=>{this.classList.remove('glitch');},2000);});}}
...
document.addEventListener('keydown',function(e){
if(e.shiftKey&&!speedrunTimer){startSpeedrun();}});
document.addEventListener('keyup',function(e){
if(e.key==='Shift'&&speedrunTimer){stopSpeedrun();}});}
